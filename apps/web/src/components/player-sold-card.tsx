...
export const PlayerSoldCard = React.forwardRef<HTMLDivElement, PlayerSoldCardProps>(
  ({ player, team, auction }, ref) => {
    return (
      <div
        ref={ref}
      ...
      >
        ...
        <div className="relative z-10 flex h-40 shrink-0 items-center justify-between border-b border-[#3c3c3c] bg-neutral-950/80 px-8 backdrop-blur-md">
          ...
          <div className="flex items-center gap-5 text-right">
            ...
            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-none border border-[#3c3c3c] bg-neutral-900 text-white">
              <HammerIcon className="h-10 w-10 text-m-blue-light" />
            </div>
          </div>
        ...
        <div className="z-10 flex min-h-0 flex-1 flex-col items-center justify-center p-8">
          <div className="relative flex w-full max-w-4xl flex-col items-center">
            {/* Player Photo Frame */}
            <div className="relative flex aspect-square w-96 items-center justify-center overflow-hidden border-2 border-[#3c3c3c] bg-neutral-900 shadow-2xl">
              {player.imageUrl ? (
                <LazyImage
                  src={player.imageUrl}
                  alt={player.name}
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-linear-to-b from-neutral-900 to-black text-neutral-600 uppercase">
                  <StarIcon className="mb-2 h-24 w-24 text-neutral-800" />
            ...
            <div className="mt-8 flex flex-col items-center justify-center text-center">
              <span className="mb-2 inline-flex items-center rounded-none border border-[#3c3c3c] bg-neutral-900 px-4 py-1.5 text-body-sm font-black tracking-[2.5px] text-white uppercase shadow-lg">
                {player.category?.name || "UNASSIGNED CATEGORY"}
              </span>
              <h1 className="text-[72px] leading-none font-black tracking-[2px] text-white uppercase">
                {player.name}
              </h1>
              <span className="mt-3 text-title-sm leading-none font-light tracking-[3px] text-[#bbbbbb] uppercase">
                {player.skills}
              </span>
            </div>
          </div>
          ...
          <div className="flex h-48 items-center justify-center border-t-2 border-[#3c3c3c] bg-neutral-950 px-8">
            <div className="flex items-center gap-8">
              ...
              {team.logoUrl ? (
                ...
                <div className="flex h-32 w-32 shrink-0 items-center justify-center rounded-none border border-[#3c3c3c] bg-neutral-900 text-[36px] font-bold text-white uppercase">
                  {team.name.slice(0, 2)}
                </div>
              )}
            </div>
